...
class Navbar extends HTMLElement {
    ...
    defaultConfig = {
        'navbar': 'navbar navbar-expand-lg navbar-light bg-white',
        'navbodyClass': 'container-fluid bg-primary',
        'brand': 'navbar-brand',
        'brandlogo': 'logo',
        'toggler': 'navbar-toggler',
        'togglerIcon': 'navbar-toggler-icon',
        'collapse': 'collapse navbar-collapse',
        'navItem': 'nav-item fw-bold',
        'ulClass': 'navbar-nav ms-5 mb-3 mb-lg-0 text-white',
        'navLink': {
            'default': 'nav-link text-white',
            'active': 'nav-link active text-dark',
            'disabled': 'nav-link disabled',
            'dropdownToggle': 'nav-link dropdown-toggle text-white',
        },
       ...
       'button': "btn btn-dark text-white border-0 px-3 py-2",
       'buttonHover': "btn btn-light text-dark border-0 px-3 py-2",
    };


    defaultData = {
        'brand': {
            'src': '/img/Brandfetch1.png',
            'text': '',
            'href': '#',
        },
        'toggler': {
    ...
    renderComponent() {
       ...
       /* collapse.appendChild(searchForm);*/
        ...
        const bookButton = this.createElement('button', this.config.button);
        bookButton.textContent = this.data.appointment.text; // Button Text
        bookButton.addEventListener('mouseover', () => {
            bookButton.className = this.config.buttonHover; // Change to hover style
        });
        bookButton.addEventListener('mouseout', () => {
            bookButton.className = this.config.button; // Revert to default style
        });
